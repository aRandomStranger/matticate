...
local function bot_leave(chat_id)
	...
	if not res then
		return 'Check the ID, I\'m pretty sure it\'s wrong.'
	else
...
local function load_lua(code)
	...
	if not output then
		output = 'Done! There was no output, however.'
	else
...
local action = function(msg, blocks)
	...
	if not blocks or not next(blocks) then
		return true
	end
	...
	if blocks[1] == 'stop' then
		db:bgsave()
		is_started = false
		api.sendReply(msg, 'matticate is shutting down...', true)
	end
	...
	if blocks[1] == 'save' then
		db:bgsave()
		api.sendMessage(msg.chat.id, 'Redis has been updated.', true)
	end
	...
	if blocks[1] == 'lua' then
		if not blocks[2] then
			api.sendReply(msg, 'Please enter a string of Lua to execute.')
			return
		end
	...
	if blocks[1] == 'download' then
		if not msg.reply then
			api.sendMessage(msg.chat.id, 'Please reply to the message that you want to download.')
		else
			local type
			local file_id
			local file_name
	...
	if blocks[1] == 'subadmin' then
		if not msg.reply then
			api.sendAdmin('Please reply to a message sent by the target user.')
			return
		end
