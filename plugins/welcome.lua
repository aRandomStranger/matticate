...
local function get_welcome(msg)
	...
	elseif type == 'custom' then
		...
		local text = 'Hello %s, welcome to *%s*!'
		text = text:format(msg.added.first_name:escape_hard(), msg.chat.title:escape_hard())
		return text
	end
end
local function action(msg, blocks)
    if blocks[1] == 'welcome' then     
        if msg.chat.type == 'private' or not roles.is_admin_cached(msg) then
        	return
        end
        ...
        if not input and msg.reply then
            ...
            if not res then
                db:hset(hash, 'type', 'no')
                db:hset(hash, 'content', 'no')
                if code == 118 then
				    api.sendMessage(msg.chat.id, 'I can\'t send that message, because it\'s too long.')
			    else
					api.sendMessage(msg.chat.id, 'This text would break the markdown. Please check your syntax and try again.\n')
			    end
            else
                local id = res.result.message_id
                api.editMessageText(msg.chat.id, id, 'Successfully changed the group\'s welcome message!', false, true)
            end
        end
    end
