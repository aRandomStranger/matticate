	...
	if next(all) then
		for unban_time,info in pairs(all) do
			if os.time() > tonumber(unban_time) then
				...
				db:srem('chat:' .. chat_id .. ':tempbanned', user_id)
			end
		end
	end
end
...
local function get_time_reply(hours)
	...
	if time_table.days ~= 0 then
		time_string = time_table.days .. 'days'
	end
	if time_table.hours ~= 0 then
		time_string = time_string .. ' ' .. time_table.hours .. 'hours'
	end
...
local action = function(msg, blocks)
	if msg.chat.type ~= 'private' then
		if roles.is_admin_cached(msg) then
		    ...
		    if not user_id then
		    	api.sendReply(msg, _(error_translation_key), true)
		    	return
		    end
		    if msg.reply and msg.reply.from.id == bot.id then
		 	...
		 	if blocks[1] == 'tempban' then
				if not msg.reply then
					api.sendReply(msg, 'Reply to someone')
					return
				end
				...
				if not temp then
					if code == 1 then
						api.sendReply(msg, 'You can directly use /ban for this!')
					else
						api.sendReply(msg, 'The time limit is one week (168 hours)')
					end
					return
				end
		    	...
		    	if not res then
					...
					if is_already_tempbanned then
						text = 'Ban time updated for %s. Ban expiration: %s'
						text = text:format(kicked, time_reply)
					else
						text = 'User %s banned. Ban expiration: %s'
						text = text:format(kicked, time_reply)
						db:sadd('chat:' .. chat_id .. ':tempbanned', user_id)
					end
		 	...
		 	if blocks[1] == 'kick' then
		    	...
		    	if not res then
		    		...
		    		local text = '%s was kicked by %s.'
		    		text = text:format(kicked, admin)
   			...
   			if blocks[1] == 'unban' then
   				api.unbanUser(chat_id, user_id)
   				local text = '%s has been unbanned by %s.'
   				text = text:format(kicked, admin)
